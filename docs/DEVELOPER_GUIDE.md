...
- boots the user-supplied `boot.rom` bundle on hardware
- produces working CPC video on Pocket
- loads the ROM bundle through an APF data slot
- outputs CPC audio
- maps Pocket controls into joystick and keyboard-style CPC input paths
- includes an experimental Dock USB keyboard input path for most common keys
- mounts `.dsk` images in Drive A and Drive B through an APF-backed translation layer to the MiSTer u765 block interface
- includes early tape support and snapshot loading
...
- Pocket savestates / Memories are supported experimentally through an
  external-PSRAM staging path with a SNA v3-compatible payload; see
...
Treat the code as a working adapter layer around real CPC machine pieces, not a polished end-state design.
...
This file is mostly platform support code. It is the "Pocket shell", not the CPC machine.
...
### 3. `src/fpga/cpc/cpc_machine_pocket.sv`
...
3. `docs/CPC_IMPORT_MANIFEST.md` if you are following imported machine files
4. `docs/ROM_ASSET_LAYOUT.md` if you are following ROM loading or model ROM assumptions
...
The authored source lives under `src/fpga` and `src/pocket`.
...
### `src/pocket/Cores/stilvoid.PocketCPC/`

Pocket package template metadata.
...
18. loaded savestate CRTC v3 timing word `0` (`[31:0]`)
19. loaded savestate CRTC v3 timing word `1` (`[63:32]`)
20. loaded savestate GA timing pack:
    `{6'd0, irq_active, int_scanline, vsync_delay, crtc_addr, crtc_v3_valid, sna_load, model}`
21. live machine CRTC v3 timing word `0` (`[31:0]`)
22. live machine CRTC v3 timing word `1` (`[63:32]`)
23. live machine GA timing pack:
    `{6'd0, irq_active, int_scanline, vsync_delay, crtc_addr, model, restore_sna_load, reset_n}`
...
failures from post-apply reset/CPU-register failures. The added timing words
also let wake failures distinguish "loaded timing state was wrong" from
"loaded timing state looked right but live video timing diverged afterwards."
...
The current savestate design and remaining validation work are tracked in
`docs/SAVESTATE_ARCHITECTURE.md` plus the user-facing notes in `README.md`
and `TODO.md`.

One important design choice is that the staged Pocket payload keeps the extra
timing state available in SNA v3. That was needed to make sleep/wake restore
reliably on hardware, especially for CRTC and related Gate Array timing state.

For the current ROM/disk/tape/snapshot transport split, see
`docs/MEDIA_TRANSPORT_ARCHITECTURE.md`.
...
- `build/package/Cores/`
- `build/package/Platforms/`

The staged `build/package/Cores/stilvoid.PocketCPC/` directory is where
generated files such as `bitstream.rbf_r`, `build-info.txt`, and the
version-stamped `core.json` now live.
...
- CPC 6128 OS, BASIC, AMSDOS, placeholder
- CPC 664 OS, BASIC, AMSDOS, placeholder
- CPC 464 OS, BASIC
...
- 128 KiB of CPC RAM
- MiSTer-style ROM bundle banks for CPC 6128, CPC 664, and CPC 464
...
`src/pocket/Cores/stilvoid.PocketCPC/input.json` tells the Pocket UI how to name and expose
the controls. Those labels are intentionally dual-purpose, because the runtime
mapping in `cpc_pocket_input.sv` treats the D-pad and face buttons as joystick
input during normal play and as keyboard helpers while the virtual keyboard is
open.

Default runtime behavior is currently:

- D-pad: joystick directions by default, or `Cursor Keys` / `QAOP` via the `D-pad Mode` core setting
- `A`: joystick fire 1
- `B`: joystick up
- `X`: `Space`
- `Y`: `Return`
...
- Start sends `Escape` during normal play
- While the VKB is open, Start arms a session-only bind mode for the highlighted key
...
While the VKB is open, the D-pad moves the selection, `L` latches `Shift`, and
`R` cycles between the two VKB pages. The second page combines function keys,
cursor keys, joystick directions/fire buttons, and a right-hand full-text
macro column for `|TAPE`, `|DISC`, `CAT`, `RUN"`, and `RUN"DISC`, each emitted
with a trailing `Return`.

The current first-pass custom remap flow is session-only:

- highlight a VKB key or joystick target
- press `Start` to arm bind mode for that key
- press one of `A/B/X/Y/L/R/Start` to assign it
- press `Select` to cancel bind mode
- the VKB shows a banner while bind mode is armed and briefly confirms the
  assigned button/key pair after a bind succeeds

Those custom bindings currently live only inside `cpc_pocket_input.sv` state,
so they disappear on reset or restart. D-pad handling stays intentionally more
constrained: the same module also exposes a menu-backed runtime preset that can
switch normal-play D-pad input between joystick directions, cursor keys, or
`QAOP`, while leaving VKB navigation behavior unchanged. The VKB second page
now also exposes joystick directions plus fire buttons `1`, `2`, and `3`, so
the same remap table can target either PS/2-backed CPC keys or MiSTer-style
joystick bits. The overlay highlights the effective current mappings, including
defaults for any Pocket buttons that have not been overridden yet, and shows
which Pocket button or buttons point at the selected target when the cursor
rests on it.
...
For the current design boundaries and tradeoffs in this subsystem, see
...
The ROM, disk, tape, and snapshot-load paths are all wired to some degree now,
but they are not equally mature. ROM loading and basic boot are the most
established. Disk is still read-only in practice. Tape and snapshot-related
flows should still be treated as early.
...
This is the project's small custom register block. It exposes the live
Pocket-facing menu/config words plus a few read-only debug words at low
addresses, such as:

- model config
- interact/menu config
- restart request toggle
- selected savestate/debug readback words
...
- `src/pocket/Cores/stilvoid.PocketCPC/video.json`
...
It resolves to the staged Pocket package under `build/package/`. Quartus only reruns
when the tracked FPGA build inputs are newer than
`build/package/Cores/stilvoid.PocketCPC/bitstream.rbf_r`.

When a rebuild is needed, it runs the Docker-based Quartus build script:

- `scripts/build_core_docker.sh`

The build emits a raw Quartus `.rbf`, then converts it into Pocket format as
`bitstream.rbf_r` by reversing the bit order in each byte.

The Docker wrapper now also:

- uses a fixed container name for status/stop/wait operations
- writes Quartus state and logs under `build/quartus/`
- writes `build/package/Cores/stilvoid.PocketCPC/build-info.txt` alongside the packaged bitstream
- updates `build/package/Cores/stilvoid.PocketCPC/bitstream.rbf_r` atomically only after a successful Quartus run

Before Quartus runs, the `Makefile` syncs the source FPGA tree into
`build/quartus/`, so Quartus can write its generated files without touching the
tracked source tree.

The `Makefile` also refreshes generated metadata in the staged
`build/package/Cores/stilvoid.PocketCPC/core.json` when git version inputs
change.

Quiet periods during fitter or timing analysis are normal. Prefer:

- `scripts/build_core_docker.sh status` to check whether the container is still running
- `scripts/build_core_docker.sh log` to inspect the most recent captured build output
- `scripts/build_core_docker.sh wait` to wait for a running build without guessing whether it hung

`make install` depends on `make dist`, so it proves the staged package can be
...
- `src/fpga/cpc/cpc_machine_pocket.sv`
