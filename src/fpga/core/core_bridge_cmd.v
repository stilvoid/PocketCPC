...
output  wire    [3:0]   debug_tstate,
output  wire    [15:0]  debug_target_status,
output  wire    [3:0]   debug_target_io
...
initial begin
    reset_n <= 0;
	    debug_tstate_r <= TARG_ST_IDLE;
	    debug_target_io_r <= 4'd0;
...
always @(posedge clk) begin
    ...
    if(bridge_wr) begin
        ...
        32'hF8xx00xx: begin
	        ...
	        32'hF8xx10xx: begin
	            case(bridge_addr[7:0])
	            8'h0: begin
	                debug_target_io_r[2] <= 1'b1;
	                target_0 <= bridge_wr_data_in; // command/status
	            end
    ...
    if(bridge_rd) begin
        ...
        32'hF8xx00xx: begin
	        ...
	        32'hF8xx10xx: begin
	            debug_target_io_r[0] <= 1'b1;
	            case(bridge_addr[7:0])
	            8'h0: begin
	                debug_target_io_r[1] <= 1'b1;
	                bridge_rd_data_out <= target_0;
	            end
	            8'h4: bridge_rd_data_out <= target_4;
	            8'h8: bridge_rd_data_out <= target_8;
	            8'h20: begin
	                debug_target_io_r[3] <= 1'b1;
	                bridge_rd_data_out <= target_20; // parameter data regs
	            end
	            8'h24: begin
	                debug_target_io_r[3] <= 1'b1;
	                bridge_rd_data_out <= target_24;
	            end
	            8'h28: begin
	                debug_target_io_r[3] <= 1'b1;
	                bridge_rd_data_out <= target_28;
	            end
	            8'h2C: begin
	                debug_target_io_r[3] <= 1'b1;
	                bridge_rd_data_out <= target_2C;
	            end
	            endcase
	        end
        32'hF8xx2xxx: begin
    ...
    ST_PARSE: begin
        ...
        default: begin
            // Pocket firmware can add host commands over time. The core
            // package may not implement all of them yet, so ignore unknown commands
            // instead of failing core startup.
            hstate <= ST_DONE_OK;
        end
        endcase
    end
    ...
    TARG_ST_WAITRESULT_RTR: begin
        debug_tstate_r <= TARG_ST_WAITRESULT_RTR;
        if(target_0[31:16] == 16'h6F6B) begin
            // done
            tstate <= TARG_ST_IDLE;
        end
...
assign debug_tstate = debug_tstate_r;
assign debug_target_status = target_0[31:16];
assign debug_target_io = debug_target_io_r;
