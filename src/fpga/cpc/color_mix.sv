//
// CPC Gate Array colour adapter from the MiSTer Amstrad CPC core.
//
...
always @(posedge clk_vid) begin
    if (ce_pix) begin
        ...
        case (mix)
            3'd0,
            3'd1: {R_out, G_out, B_out} <= rgb;
            3'd2: {       G_out       } <= {          px[15:8]            };
            3'd3: {R_out, G_out       } <= {px[15:8], px[15:8] - px[15:10]};
            3'd4: {       G_out, B_out} <= {          px[15:8], px[15:8]  };
            3'd5: {R_out, G_out, B_out} <= {px[15:8], px[15:8], px[15:8]  };
        endcase
