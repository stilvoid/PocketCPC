...
FPGA_DIR="$SKELETON/src/fpga"
CORE_DIR="$SKELETON/Cores/steve.AmstradCPC"
OUTPUT_DIR="$FPGA_DIR/output_files"
STATE_DIR="$OUTPUT_DIR/build_monitor"
...
PACKAGED_BITSTREAM="$CORE_DIR/bitstream.rbf_r"
PACKAGED_BITSTREAM_TMP="$PACKAGED_BITSTREAM.tmp"
BUILD_INFO_FILE="$CORE_DIR/build-info.txt"
...
assert_fresh() {
    ...
    stale="$(find_stale_sources)"
    if [ -n "$stale" ]; then
        echo "Packaged bitstream is older than Quartus input files; run make build-skeleton first." >&2
        echo "$stale" | sed 's#^#  #'
        exit 1
    fi
}
